...
void main() {
  ...
  test('automatic barge-in stops TTS and sends detected speech', () async {
    ...
    speech.add(const SpeechEvent.result('miert', false));
    await Future<void>.delayed(Duration.zero);

    expect(tts.stopCount, 1);
    expect(controller.state, VoiceState.listening);

    speech.add(const SpeechEvent.result('miert fontos', true));
    await Future<void>.delayed(Duration.zero);

    expect(sent, ['miert fontos']);
    expect(controller.state, VoiceState.idle);
    await speaking;
  });

  test('automatic barge-in ignores spoken answer echo', () async {
    final speech = _ManualSpeechAdapter();
    final tts = _StoppingBlockingTtsAdapter();
    final sent = <String>[];
    final controller = VoiceController(
    ...
    speech.add(const SpeechEvent.result('egy hosszabb valasz', false));
    await Future<void>.delayed(Duration.zero);
  ...
  test('automatic barge-in restarts after no match while TTS speaks', () async {
    final speech = _ManualSpeechAdapter();
    final tts = _StoppingBlockingTtsAdapter();
    final controller = VoiceController(
      speech: speech,
    ...
    speech.add(const SpeechEvent.error('error_no_match'));
...
class _StoppingBlockingTtsAdapter extends FakeTtsAdapter {
  final _completion = Completer<void>();
  var started = false;
...
class _ManualSpeechAdapter implements SpeechAdapter {
  final locales = <String>[];
  final _controllers = <StreamController<SpeechEvent>>[];
  var stopCount = 0;
