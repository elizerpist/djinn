...
void main() {
  ...
  test(
    'keeps short headings with following paragraph instead of standalone chunks',
    () {
      final chunks = const LocalChunkBuilder().build(
        documentId: 'doc-copd',
        pages: const [
  ...
  test('classifies score-like OCR blocks as table chunks', () {
    final chunks = const LocalChunkBuilder().build(
    ...
    final scoreLikeChunk = chunks.singleWhere(
      (chunk) => chunk.text.contains('RAVE score'),
    );

    expect(scoreLikeChunk.kind, LocalChunkKind.table);
  });
}
