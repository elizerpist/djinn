...
- [x] Replace the current V4-path assertions with assertions that `index.html` has one Universe button with `data-route="universe"`, `assets/app.js` maps `universe` to `screens/universe.html`, and the app imports only `initUniverse4` for that route.
- [x] Assert that `app.js` contains no public route entries for `universe-morph-test`, `universe-morph-test-v2`, `universe-morph-test-v3`, or `universe-morph-test-v4`, and that `screens/universe.html` contains no version selector/tabs.
- [x] Assert that child menus link to `universe` and render the label `Universe`, not a version-specific route.
- [x] Run the new and updated tests first and verify they fail against the current implementation before editing runtime files.
...
- [x] Change `routes` and `activeNav` so `universe` is the sole Universe route and no route normalization can silently resurrect a deleted version.
- [x] Make `render()` initialize `initUniverse4` only when `normalized === 'universe'`; remove the legacy `initUniverseMorphTest` import and all V1/V2/V3 initialization branches.
- [x] Change the bottom navigation and all cross-screen links to `data-route="universe"`.
- [x] Remove the legacy `universe-morph-test.css` stylesheet from `index.html` after its required rules are migrated in Task 4; keep the V4 stylesheet revision explicit.
- [x] Rename the copied V4 screen's public title and controls to “Universe” while retaining the existing stage shell, reset, fullscreen, debug, Force background, Globe background, and G6 stage hooks.
- [x] Keep the route screen's internal V4 stage attributes (`data-universe4-*`) unchanged unless the adapter contract requires a deliberate rename.
...
- [x] Inventory the imports from `assets/universe-morph-model.js` used by the V4 U3 path (seed/data construction, focus distance/target math, orbit clamping, pointer classification, easing, projection, and surface arc generation).
- [x] Move those pure responsibilities into the V4 model owner, preserving function names/behavior at the V4 boundary and keeping deterministic snapshots frozen and immutable.
- [x] Update `universe4-u3-inline-source.js` and the consolidated Force stage to import only the V4-owned model; do not leave a compatibility import from `../universe-morph-model.js`.
- [x] Add tests for deterministic model output, focus-camera math, pointer classification thresholds, and surface-arc sampling. Make the tests fail before the extraction and pass after it.
- [x] Do not move Explore V5/V7 data or the canonical Globe selection policy into the Force model; those remain in their existing Explore/U4 owners.
...
- [x] Extract the V4-needed portions of `assets/universe-morph-test.js` into the V4 Force owner: ForceGraph creation, galaxy node rendering, selected-planet focus/morph, V3-reference lighting/material setup, handoff frame/render-profile capture, reset, input enable/disable, pause/resume, background color, and disposal.
- [x] Preserve the adapter surface consumed by `createUniverse4U3Stage`: `setOpacity`, `setInputEnabled`, `pauseAnimation`, `resumeAnimation`, `resetToGalaxyOverview`, `setBackgroundColor`, `resumeInlineInteraction`, `captureHandoffFrame`, `captureRenderProfile`, `applyHandoffCamera`, and `dispose`, plus `onPlanetReady`/helper callbacks.
- [x] Keep one ForceGraph animation loop and stop its vendor loop, HUD RAF, and cosmic environment whenever Globe.gl is active; resume them only for reset/return-to-planet.
- [x] Replace `initUniverseMorphTest` in `universe4-u3-stage.js` with the V4 Force owner and V4-specific root/class names. Preserve the handoff matcher’s measured CSS center/radius and landmark data.
- [x] Add source-contract assertions that `universe4-u3-stage.js` has no `initUniverseMorphTest` import, the V4 Force owner exports the adapter seams, and no runtime file imports `universe-morph-test.js`.
- [x] Run focused Force/U4 handoff tests before broad tests; inspect the browser trace for the existing `u4.u3.inline.complete` and `u4.handoff.complete` events.
...
- [x] Run `rg -n "universe-morph-test|universe-morph-model|Universe v[123]" prototypes/working-prototype/assets prototypes/working-prototype/screens prototypes/working-prototype/tests` and classify every match as a required migration, an allowed historical test fixture, or a removable stale reference.
- [x] Remove all public route/screen/import references; no deleted file may remain reachable through `app.js`, `index.html`, a screen link, or a dynamic import.
- [x] Remove version tabs and stale labels from the copied canonical screen without touching the internal V7 Explore variant.
- [x] Confirm the source tree has exactly one public Universe screen and one V4 Force/Globe/G6 composition.
...
- [x] Increment every browser cache-buster for a changed module and update exact-revision contract tests in the same change.
- [x] Keep the existing performance/debug contracts (throttled U4 diagnostics, camera-only visual refresh, reused labels, paused hidden renderer) green.
- [x] Verify repeated root-city and context-city taps still follow the current U4 policy: first foreign/context tap clears or focuses as defined, repeated tap enters the embedded map, and map reset/back returns to the planet without a second renderer.
...
- [x] Run every prototype Node contract test from the prototype directory:
  ```sh
...
- [x] Run the dependency/public-surface scan:
  ```sh
...
- [x] Start/use the local HTTP server on port 8082 and verify `GET /index.html`, `GET /screens/universe.html`, and the V4 module tree return 200 responses.
- [x] Exercise the browser flow with the existing debug trace: overview -> focused planet -> Globe.gl -> repeated city tap -> embedded G6/Google Maps view -> reset/back. Confirm no uncaught `map.error`, `pointer.tap.reject` on a deliberate stationary city tap, or runaway controls/RAF logs.
- [x] Mark U4-01..U4-08 `DONE` only when the corresponding evidence is present; otherwise leave the item `PARTIAL`/`BLOCKED` and report it before commit.
...
- [x] Commit the implementation as a focused migration commit after verification, using a message such as `refactor: make Universe V4 the single public universe`.
- [x] Push only when explicitly requested by the user; report the commit hash, branch, test command, and any remaining untracked files (including `.codex/` metadata) without hiding them.
