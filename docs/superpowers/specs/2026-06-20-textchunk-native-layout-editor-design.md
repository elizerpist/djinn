...
## 2026-06-21 Thirteenth Native Selection Pipeline Checklist

Source: user message on 2026-06-21 with Perplexity analysis: the rail no longer grows back, but handle drag still ticks because the editor still interferes with Flutter's native selection pipeline. During a native Android/Flutter selection handle drag, the editor must not wrap handle gestures, mutate `EditableText` layout, or write `controller.selection`; the contextual rail must be separate from native drag mechanics.

| ID | Source | Intended Code Area | Acceptance Condition | Verification Method | Status |
| --- | --- | --- | --- | --- | --- |
| TC-REQ-062 | User 2026-06-21: "natív android flutter kijelölést akarok" plus Perplexity: "ne customize selection handles" | `text_chunk_canvas_editor.dart`, selection controls | Native selection handles are not wrapped in an extra `Listener`/pointer route that observes handle down/move/up/cancel. The editor uses Flutter's built-in handle gesture path and detects drag from `onSelectionChanged` only. | DONE: code inspection shows `_TextChunkSelectionControls`, pointer routes, and handle keys removed; widget test `native selection handles are not wrapped by custom listeners`; full `note_text_chunk_editor_screen_test.dart` passed 30/30; `flutter analyze` clean; full `flutter test` passed 413 with 8 environment skips | DONE |
| TC-REQ-063 | Perplexity: "`controller.selection = normalizedSelection` a SelectionChangedCause.drag ágban nagyon gyanús" | `text_chunk_canvas_editor.dart`, selection changed handling | The `SelectionChangedCause.drag` path does not assign to `controller.selection`, normalize offsets, show toolbar, or otherwise compete with Flutter's internal drag calculation. It only records a rail-hidden/drag-active state outside the selection pipeline. | DONE: widget test `native drag callback does not mutate selection or editable presentation`; code inspection of `_handleNativeSelectionChanged`; full `note_text_chunk_editor_screen_test.dart` passed 30/30; full `flutter test` passed 413 with 8 environment skips | DONE |
| TC-REQ-064 | Perplexity: "drag alatt semmilyen layout-módosítás" and user logs with ticks despite rail hidden | `text_chunk_canvas_editor.dart`, rail placeholder/layout state | Once a native selection drag starts, the editor must not change the `EditableText` placeholder/text-span presentation until a later non-drag selection action finishes the interaction. Any rail hiding during drag must be visual-only/outside the editable layout. | DONE: tests `selection handle drag keeps rail hidden until explicit release`, `native drag callback does not mutate selection or editable presentation`, and `native drag after repeated paragraph stepping keeps native margin stable` assert stable native plain text/presentation during drag; full `note_text_chunk_editor_screen_test.dart` passed 30/30 | DONE |
| TC-REQ-065 | Perplexity: "contextual rail kívül legyen az EditableText layoutján" | rail host/overlay positioning | The contextual rail is no longer inserted into the active editable text span while native selection handles are active. It is shown only after non-drag selection settles and hidden during native drag without altering native text layout. | DONE: rail widget remains a `Stack` overlay, not a native handle child or `EditableText` child; drag hides `note-text-inline-selection-rail` while preserving native presentation; full `note_text_chunk_editor_screen_test.dart` passed 30/30; full `flutter test` passed 413 with 8 environment skips | DONE |

## 2026-06-21 Fourteenth Native Editable Purity Checklist

Source: user message on 2026-06-21 with 19:59 logs: "ugyanúgy nem lehet kijelölni rendesen ... rail eltűnik, de a szöveg nem megy vissza". The previous fix hid the visible rail but left `rail-line-*` invisible placeholders inside the active `EditableText` span (`controllerLen=120`, `plainLen=134`, `placeholderDelta=14`). This checklist supersedes the placeholder-preserving part of TC-REQ-064/065: active native selection must operate on the real text only.

| ID | Source | Intended Code Area | Acceptance Condition | Verification Method | Status |
| --- | --- | --- | --- | --- | --- |
| TC-REQ-066 | User 19:59 logs showing `selectionDragActive=true` with `placeholderDelta=14` and `placeholders=[rail-line-*]` | `text_chunk_canvas_editor.dart`, native placeholder construction | Whenever the native editable has an active non-collapsed selection or a native selection drag is active, the `EditableText` span contains no rail placeholder: `plainLen == controllerLen`, `placeholderDelta == 0`, and no `rail-line-*` placeholder is configured. | DONE: RED/GREEN widget test `native selection and drag use real text without rail placeholders`; debug assertions check `placeholderDelta=0`, no `rail-line-*`, no `note-text-inline-selection-spacer`; `note_text_chunk_editor_screen_test.dart` passed 31/31; full `flutter test` passed 414 with 8 environment skips; `flutter analyze` clean | DONE |
| TC-REQ-067 | User: "rail eltűnik, de a szöveg nem megy vissza" | `text_chunk_canvas_editor.dart`, rail visual/spacer rendering | When handle drag begins, the visible rail is hidden and the text returns to its normal native layout immediately; no invisible spacer remains under the selected line. | DONE: tests `selection handle drag keeps rail hidden until explicit release`, `rail overlay leaves the actual native editable text unchanged`, and `native selection and drag use real text without rail placeholders` assert real native plain text and no spacer before/during drag | DONE |
| TC-REQ-068 | User: "natív text edit élmény" | `text_chunk_canvas_editor.dart`, rail architecture | The selection rail is a visual overlay only. It must not reserve space in the native editable text span while selection handles are usable. | DONE: code inspection shows rail placeholder insertion removed; rail remains a `Stack` overlay; overlay positioning still accounts for visual underline lanes without adding native placeholders; widget tests show the rail can be visible while the spacer key is absent | DONE |
