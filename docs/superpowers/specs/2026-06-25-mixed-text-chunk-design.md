...
| --- | --- | --- | --- | --- | --- |
| OCR-01 | User: extracted text is unreadable because OCR/PDF text breaks every word onto a new line | `lib/src/knowledge/ui/manual_pdf_region_text.dart` | Selected PDF text joins fragments into readable lines and paragraphs instead of one word per line. | Unit tests with word fragments on same line, bullet lines, and paragraph gaps. | DONE |
| OCR-02 | User expected the first PDF paragraph/list structure from raw PDF | `manual_pdf_region_text.dart`, mixed parser helpers | Bullet markers and numbered markers remain visible and are not separated from their text. | Unit tests from representative first-section fixture text. | DONE |
| MIX-01 | User: "kozos chunk modell az appba" | `lib/src/notes/models/note_document.dart` | A new mixed block type serializes/deserializes paragraph, list, and table sections in order. | `note_document_test.dart` model round-trip tests. | DONE |
| MIX-02 | User: "egyelore uj elem legyen, ne torold a korabbi chunk modellt" | note model, adapters, editors | Existing text/list/table/flowchart blocks still parse, render, edit, index, and export through legacy paths. | Existing regression tests plus targeted compatibility tests. | DONE |
| MIX-03 | User: textchunk should accept lists | mixed editor | Mixed editor can add, edit, delete, indent/outdent, reorder, and tag list items inside the chunk. | Mixed editor widget tests based on current list editor tests. | DONE |
| MIX-04 | User: list behavior must match listchunk | mixed editor list section | List items have draggable cards, dynamic bullets/checkbox/hierarchy behavior, and list hierarchy controls. | Widget tests for drag reorder and hierarchy markers. | DONE |
| MIX-05 | User: tables should integrate into textchunk | mixed editor table section | Mixed editor can add table sections and use current row/column/cell edit, resize, reorder, and scoped tag behavior. | Mixed table widget tests adapted from current table editor tests. | PARTIAL |
| MIX-06 | User: text with explanatory sentences, list, and table should be one logical unit | mixed editor, model plain text | Mixed chunk plain text and indexing preserve the ordered paragraph/list/table content as one chunk. | Model tests for `plainText`, `displayTextForIndexing`, and search metadata. | DONE |
| UX-01 | User: rail should not be inline, it should be above keyboard like textchunk | mixed editor rail | Text, list, and table selection actions in the mixed editor use a keyboard-top rail, not inline action rails inside list/table cards. | Widget tests with `viewInsets.bottom` and selected text/list/table targets. | DONE |
| UX-02 | User: editor layout should satisfy text layout criteria and resemble PDF structure | mixed editor | Mixed editor uses full-width readable content, paragraph spacing, bullet/list indentation, and table blocks without forcing all text into a narrow column. | Widget tests for field/card widths; manual screenshot review. | PARTIAL |
| PDF-01 | User: new behavior should apply after manual PDF text chunk extraction | `manual_chunk_editor_screen.dart`, PDF chunk adapter/repository | Manual PDF text chunks are created as mixed chunks with structured content when possible. | Widget/repository tests that saved manual chunks retain mixed section JSON. | DONE |
| PDF-02 | User wants only 3 effective chunk types going forward | add chunk UI, shared cards | New add/manual creation treats text as the container for paragraphs/lists/tables; legacy list/table creation is no longer primary for new chunks. | Widget tests for FAB/manual sheet options and shared card labels. | PARTIAL |
| COMPAT-01 | Existing PDF chunks store only `text` and `chunkKind` | `pdf_chunk_note_block_adapter.dart`, repository model | Legacy PDF list/table/text chunks can be opened and optionally converted to mixed without data loss. | Adapter tests for text/list/table to mixed conversion. | DONE |
...
| UX-06 | User 2026-06-26: "egy rail lesz... top keyboard, ha nincs keyboard, akkor bottomscreen" | `note_mixed_text_chunk_editor_screen.dart`, `note_tag_pills.dart` | Mixed editor uses one universal keyboard/bottom rail only; it starts with tag add/delete and tag-chevron controls, then vertical separators and formatting/list/indent/design sections. | Widget tests for rail keys, placement with keyboard inset, and absence of inline rails. | DONE |
| UX-07 | User 2026-06-26: "a headerbe egyedül egy táblázat beszúrása gomb lesz" | `NoteChunkEditorHeader` usage in mixed editor | Mixed editor header exposes only the insert-table action; paragraph/list insertion moves to rail formatting/conversion, not header buttons. | Widget test asserts add paragraph/list keys absent and add table key present. | DONE |
| UX-08 | User 2026-06-26: "nincs sem szövegblokk sem listablokk" | mixed editor section rendering | Mixed editor renders one continuous note surface without section drag areas or right-side three-dot menus; table is the only visible in-note container. | Widget tests assert no `ReorderableDragStartListener`/section popup menu in mixed editor. | DONE |
| UX-09 | User 2026-06-26: heading button expands upward with 3 heading levels | mixed rail, mixed section model | Heading rail action opens an upward panel with three heading choices; each persists heading role and selected level for distinct size/line-height/bold styling. | Widget/model tests for popover keys, JSON round-trip and paragraph style. | DONE |
| UX-10 | User 2026-06-26: text color, underline color and background color expand to palette and close on choice | mixed rail, mixed section model | Color actions open a palette above the rail; choosing a color persists it on the active note target and closes the palette. | Widget/model tests for color popover and JSON round-trip. | PARTIAL |
| UX-11 | User 2026-06-26: text, dynamic hierarchy list, non-dynamic hierarchy list are separate rail buttons | mixed rail/list conversion | Rail exposes separate text, dynamic-list and static-list actions; selected paragraph/list target can switch modes without leaving the editor. | Widget tests for keys and conversion/mode changes. | DONE |
| UX-12 | User 2026-06-26: universal step left/right for paragraph and list hierarchy | mixed rail and model | Indent/outdent works for active paragraph as one paragraph unit and for list items as hierarchy level changes. | Widget tests for paragraphIndentLevel and list item level changes. | DONE |
| UX-13 | User 2026-06-26: table design/layout 100% from table chunk; table scrolls horizontally only when wider | mixed table renderer, table editor shared behavior | Mixed table uses the legacy table editor visual layout, row/column/cell operations and rail design toggles; horizontal scroll is scoped to the table, not the whole text canvas. | Widget tests for table rail actions/design toggles and manual screenshot review. | PARTIAL |
| UX-14 | User 2026-06-26: insert table at cursor height as 2x2; table context adds row/column add/delete and table delete | mixed editor table insertion and rail | Add table inserts after the active target with a 2x2 table; table selection adds row/column add/delete and delete-table controls to the universal rail. | Widget tests for section order, 2x2 rows, and table-specific rail keys. | DONE |
| UX-15 | User 2026-06-26: "ha a user listat steppel, akkor minden lepes ujabb designt alkalmazzon a lista jelolesnek ... mother viszont mindig szam legyen" | shared list hierarchy marker helper, `note_mixed_text_chunk_editor_screen.dart`, `note_list_chunk_editor_screen.dart`, `note_pdf_document_builder.dart` | In hierarchical list mode, level-0 mother items are numbered in order while each nested step uses a distinct marker design; checkbox/non-hierarchical list modes do not use these hierarchy markers. | Widget tests for mixed and legacy list editors with level-0, level-1, level-2, and level-3 items; PDF marker helper test for the same sequence. | DONE |
...
- UX-10 is PARTIAL because the universal palette and persistence path exists, but styling is still section-scoped rather than true per-selection/per-cell rich text spans.
- UX-13 is PARTIAL because the mixed table now uses the legacy table visual language, scoped horizontal scrolling, row/column operations, and rail design toggles, but full legacy table parity such as row/column resizing is still not extracted into the mixed editor.
