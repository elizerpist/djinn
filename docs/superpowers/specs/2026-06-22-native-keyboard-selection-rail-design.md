...
| NKR-006 | User: "keyboard tetején... slide közben" and "egy frame késést sem akarok" | Android native rail host | Android rail is a native view positioned from `WindowInsetsAnimation.Callback`, not a Flutter `viewInsets` follower. | Kotlin code inspection and Android CI build. | PARTIAL |
...
| NKR-009 | Project Flutter build constraint | GitHub Actions | Android debug APK builds online; no local Termux APK build. | GitHub Actions URL after push. | PARTIAL |
